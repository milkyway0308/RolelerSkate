...
public class RolelerSkate {
    ...
    private static final String VERSION = "Alpha 0.0.22";
...
//                            rl.delete();
            ...
            } else if (message.getContent().startsWith("Music.setRepeat(")) {
                if (!message.getContent().endsWith(");"))
                    return;
                String substring = message.getContent();
                substring = substring.substring(substring.indexOf("(") + 1, substring.lastIndexOf(")")).trim();
                if (!substring.equals("true") && !substring.equals("false")) {
                    message.getChannel().block().createEmbed(spec -> {
                        spec.setColor(Color.RED)
                                .addField("오류!", "파라미터는 true 혹은 false만 허용됩니다.", false);
                    }).block();
                    return;
                }
                boolean parsed = Boolean.parseBoolean(substring);
                message.delete().block();
                synchronized (RolelerSkate.class) {
                    ThinkingChair chair = THINKING.get(message.getGuildId().get());
                    if (chair != null) {
                        chair.setRepeat(parsed);
                        if (parsed) {
                            message.getChannel().block().createEmbed(spec -> {
                                spec.setColor(Color.GREEN)
